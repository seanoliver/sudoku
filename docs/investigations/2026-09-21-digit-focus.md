...
Only shortcut discovery is saved, under `sudoku.focus-hold-learned.v1`. Read/write failure is tolerated. Focus itself resets on reload or a new puzzle. A stable-height row prevents the board moving when the hint disappears or focus changes.
