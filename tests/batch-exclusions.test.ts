...
test('batch exclusions are additive across mixed cells and undo atomically', () => {
  ...
  const after = engine.addExclusions(before, { indices: [0, 1, 2, 2, 80], value: 4 });
...
test('batch exclusions skip filled cells, invalid targets and no-ops', () => {
  ...
  assert.equal(engine.addExclusions(entered, { indices: [given, empty, -1, 81, NaN, 1.5], value: 4 }), entered);
  assert.equal(engine.addExclusions(entered, { indices: [], value: 4 }), entered);
  for (const value of [0, -1, 10, NaN, 1.5]) {
    assert.equal(engine.addExclusions(entered, { indices: [0], value }), entered);
  }
  const complete = { ...puzzle, values: [...puzzle.solution] };
  assert.equal(engine.addExclusions(complete, { indices: [0], value: 4 }), complete);
});
...
test('batch exclusions preserve other exclusions and empty manual ownership through undo', () => {
  ...
  const after = engine.addExclusions(before, { indices: [0, 1], value: 4 });
