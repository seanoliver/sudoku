...
test('single and batch annotations remain unrestricted', () => {
  const game = enter(blank(), { index: 4, value: 4 });
  for (const mode of [{ pencil: true }, { exclude: true }]) {
    const annotated = enter(game, { index: 0, value: 4, filterNumberKeys: true, ...mode });
    assert.deepEqual(mode.pencil ? annotated.notes[0] : annotated.exclusions[0], [4]);
    assert.deepEqual(enter(annotated, { index: 0, value: 4, filterNumberKeys: true, ...mode }).notes[0], []);
    assert.deepEqual(enter(annotated, { index: 0, value: 4, filterNumberKeys: true, ...mode }).exclusions[0], []);
  }
