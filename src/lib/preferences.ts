...
export function restorePreferences(raw: string | null): Preferences {
  try {
    ...
    if (prefs && ['system','light','dark'].includes(prefs.theme) && typeof blocking === 'boolean' && typeof prefs.highlightPeers === 'boolean') {
      return { theme: prefs.theme, blockIncorrectAnswers: blocking, highlightPeers: prefs.highlightPeers, smartHighlighting: typeof prefs.smartHighlighting === 'boolean' ? prefs.smartHighlighting : false, filterNumberKeys: typeof prefs.filterNumberKeys === 'boolean' ? prefs.filterNumberKeys : false, hideTimer: typeof prefs.hideTimer === 'boolean' ? prefs.hideTimer : false };
    }
