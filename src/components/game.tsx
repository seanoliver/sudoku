...
import { createGame, restartGame, enter, addNotes, addExclusions, fillNotes, undo, restore, isComplete, SAVE_KEY, type GameState } from '@/lib/game';
...
export default function SudokuGame() {
  ...
  const filtering = preferences.filterNumberKeys && !pencil && !excluding;
      ...
      {complete ? <div className="completion" role="status"><span className="success-mark"><Icon name="check" size={25}/></span><div><h2>Nicely done.</h2><p>Every number in its place.</p></div><button className="primary-button" onClick={() => openSheet('new')}>Play again</button></div> : <>
        ...
        <div className={`number-pad ${pencil || excluding ? 'pencil-mode' : ''}`} aria-label="Number pad">
          {DIGITS.map(n => {
            ...
            return <button key={n} className={`number-key ${!remaining ? 'digit-finished' : ''} ${filtered ? 'digit-filtered' : ''}`} aria-label={batchSelection ? `${excluding ? 'Exclude' : 'Add note'} ${n} ${excluding ? 'from' : 'to'} ${selection.indices.length} selected cells` : `${excluding ? 'Rule out' : 'Enter'} ${n}${pencil ? ' as a note' : ''}${filtered ? ', unavailable: already in this row, column, or box' : ''}`} disabled={!editable || filtered} onClick={() => input(n)}><span>{n}</span>{!batchSelection && <small aria-hidden="true">{remaining || <Icon name="check" size={10}/>}</small>}</button>;
          })}
        </div>
        <p className="input-hint" role="status">{batchSelection ? excluding ? 'Tap a number to exclude from all selected cells.' : 'Tap a number to add a note to all selected cells.' : blockedEntry && blockedEntry.index === selected && !pencil && !excluding && (blockedEntry.kind === 'constraint' ? filtering : preferences.blockIncorrectAnswers) ? blockedEntry.kind === 'constraint' ? `${blockedEntry.value} is already in this row, column, or box.` : `${blockedEntry.value} is incorrect for this cell. Answer blocked.` : excluding ? 'Exclude on. Tap a number to rule it out or restore it.' : pencil ? 'Notes on. Tap a number to add or remove a note.' : game?.givens[selected] ? 'Choose an empty cell to add a number.' : filtering ? entryDigits.length ? 'Dimmed numbers already appear in this row, column, or box.' : 'No numbers available here. Check nearby entries or undo.' : 'Select a cell, then a number. Drag across empty cells to select.'}</p>
        </div>
      </>}
      ...
      <div className="sheet-content"><div className="sheet-handle"/><button className="sheet-close icon-button" onClick={closeSheet} aria-label="Close dialog"><Icon name="close" size={19}/></button>
        ...
        {sheet === 'settings' && <><h2 id="sheet-title">Settings</h2>
          <div className="puzzle-actions">
            ...
            <button className="puzzle-action secondary-puzzle-action" disabled={!game?.history.length || paused || busy || complete} onClick={() => { doUndo(); closeSheet(); }} title="Undo (⌘Z / Ctrl+Z)"><Icon name="undo" size={20}/><span>Undo</span></button>
        ...
        {sheet === 'help' && <><div className="sheet-symbol"><Icon name="help" size={28}/></div><h2 id="sheet-title">Nine numbers. One rule.</h2><p className="sheet-subtitle">Fill every row, column, and 3 × 3 box with the numbers 1–9, using each number just once.</p><div className="help-row"><Icon name="pencil"/><div><h3>Room for a possibility</h3><p>Drag from an empty cell to select cells immediately, or hold an empty cell to begin. Drag across other empty cells, or tap them to add or remove them from your selection. Notes turns on for your selection. Toggle Exclude to choose exclusions, then tap a number to apply it to all selected cells and return to normal entry. Exclude keeps already-excluded digits excluded. Tap a filled cell to finish selecting, or use Clear selection or Escape. These actions leave your notes unchanged. For repeated note entry in one cell, use the Notes switch. Entering a number clears that note from related cells.</p></div></div><div className="help-row"><Icon name="undo"/><div><h3>Try things out</h3><p>Undo in Settings takes back your last change, including notes. Erase appears when the selected cell contains your number or annotations and clears that cell. The darker starting numbers stay in place.</p></div></div><div className="help-row"><Icon name="help"/><div><h3>Smart highlighting</h3><p>Enable Smart highlighting in Settings, then select a filled cell. Green cells show where its number is allowed by the current row, column, and box, excluding digits you have ruled out. These are possible placements, not guaranteed answers. Select an empty cell to clear the highlights when no digit is focused.</p></div></div><div className="help-row"><Icon name="help"/><div><h3>Keep a number in focus</h3><p>Select a filled cell and choose Focus on its number, or hold the filled cell. Matching numbers and notes stay highlighted while you select empty cells or add notes. With Smart highlighting enabled, possible cells stay green too. Select another filled number to switch. Clear focus stops focusing; Clear selection only clears your batch. Focus resets for a new puzzle or when you reopen the app.</p></div></div><div className="help-row"><Icon name="pencil"/><div><h3>Fill notes when you want</h3><p>Fill notes in Settings replaces notes in every empty cell with all possibilities allowed by placed numbers. Your crossed-out exclusions stay in place and are left out of the generated notes. Generated notes are blue; notes you edit use the normal text color. Undo restores the previous notes. Entering a number clears matching notes from related cells. Notes do not otherwise update automatically.</p></div></div><div className="help-row"><Icon name="pencil"/><div><h3>Record what you rule out</h3><p>Turn on Notes, then Exclude, and tap a number to cross it out in a cell. Tap again to clear it. Exclusions also remove that cell from Smart highlighting for that number. They never trigger deductions in other cells.</p></div></div><div className="help-row"><Icon name="filter"/><div><h3>Filter number keys</h3><p>Enable Filter number keys in Settings to dim and disable numbers already in the selected cell’s row, column, or box. Keyboard entry follows the same rule. Notes and exclusions stay unrestricted. Available numbers are possibilities; Block incorrect answers checks them against the solution separately.</p></div></div><div className="keyboard-help"><h3>Using a keyboard?</h3><p><kbd>↑ ↓ ← →</kbd> Move between cells</p><p><kbd>1–9</kbd> Enter a number <kbd>N</kbd> Toggle notes</p><p><kbd>X</kbd> Toggle exclusions while Notes is on</p><p><kbd>⌫</kbd> Erase <kbd>⌘ / Ctrl Z</kbd> Undo</p></div><button className="primary-button full-width" onClick={closeSheet}>Got it</button></>}
