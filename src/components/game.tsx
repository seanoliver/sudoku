...
import { automaticNotes, conflicts, peers, possibleCells, type Difficulty, type Puzzle } from '@/lib/sudoku';
...
export default function SudokuGame() {
  ...
  const possible = useMemo(() => game && preferences.smartHighlighting && !complete
    ? possibleCells(game.values, selectedValue) : new Set<number>(), [game, preferences.smartHighlighting, complete, selectedValue]);
  const values = game?.values;
  const autoNotes = useMemo(() => preferences.autoNotes && values
    ? automaticNotes(values) : null, [preferences.autoNotes, values]);
