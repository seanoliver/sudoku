...
export default function SudokuGame() {
  ...
  const selection = useNoteSelection({
    ...
    onSelect: setSelected, onBegin: () => setNoteMode('value'),
  });
  ...
  const input = (value: number) => {
    ...
    } else setGame(current => current ? enter(current, { index: selected, value, pencil, exclude: excluding }) : current);
  };
  const doUndo = () => { if (!paused && !busy && !complete) { resetSelection(); setGame(current => current ? undo(current) : current); } };
  const handleKey = (event: KeyboardEvent) => {
    ...
    if (event.key in movement) {
      ...
      resetSelection(); setSelected(next); board.current?.querySelector<HTMLButtonElement>(`[data-index="${next}"]`)?.focus();
  ...
  return <div className="app" onKeyDown={handleKey}>
    ...
    <main className="game">
      ...
      {complete ? <div className="completion" role="status"><span className="success-mark"><Icon name="check" size={25}/></span><div><h2>Nicely done.</h2><p>Every number in its place.</p></div><button className="primary-button" onClick={() => openSheet('new')}>Play again</button></div> : <>
        ...
        <p className="input-hint" role="status">{batchNotes ? `${selection.indices.length} ${selection.indices.length === 1 ? 'cell' : 'cells'} selected. Tap a number to add a note.` : excluding ? 'Exclude on. Tap a number to rule it out or restore it.' : pencil ? 'Notes on. Tap a number to add or remove a note.' : game?.givens[selected] ? 'Choose an empty cell to add a number.' : 'Select a cell, then a number. Hold a cell for notes.'}</p>
      </>}
