...
import { createGame, enter, fillNotes, undo, restore, isComplete, SAVE_KEY, type GameState } from '@/lib/game';
...
export default function SudokuGame() {
  ...
  const pencil = noteMode === 'note';
  const excluding = noteMode === 'exclude';
  ...
  const input = (value: number) => {
    ...
    setGame(current => current ? enter(current, { index: selected, value, pencil, exclude: excluding }) : current);
  };
  ...
  const handleKey = (event: KeyboardEvent) => {
    ...
    else if (event.key.toLowerCase() === 'x') { event.preventDefault(); setNoteMode(mode => mode === 'exclude' ? 'value' : 'exclude'); }
    else if (event.key.toLowerCase() === 'n') { event.preventDefault(); setNoteMode(mode => mode === 'note' ? 'value' : 'note'); }
  };
    ...
    <main className="game">
          ...
          {Array.from({ length: 9 }, (_, row) => <div role="row" className="board-row" key={row}>
            {Array.from({ length: 9 }, (_, col) => {
              ...
              return <div role="gridcell" aria-selected={selectedCell} aria-readonly={given} aria-rowindex={row+1} aria-colindex={col+1} key={i} className="cell-slot"><button className={classes} data-index={i} data-given={given} tabIndex={selected === i ? 0 : -1} aria-label={`Row ${row+1}, column ${col+1}, ${value ? `${value}${given ? ', given' : ''}` : notes.length ? `${generated ? 'generated notes' : 'notes'} ${notes.join(', ')}` : 'empty'}${!value && ruledOut.length ? `, ruled out ${ruledOut.join(', ')}` : ''}${possible.has(i) ? `, possible placement for ${selectedValue}` : ''}${badCells.has(i) ? ', conflict' : ''}`} aria-disabled={complete} onClick={() => setSelected(i)}>
                ...
                <CellNotes key={game?.id} filled={Boolean(value)} manual={generated ? EMPTY_NOTES : notes} automatic={generated ? notes : EMPTY_NOTES} excluded={ruledOut} boardKey={boardKey}/>
      ...
      {complete ? <div className="completion" role="status"><span className="success-mark"><Icon name="check" size={25}/></span><div><h2>Nicely done.</h2><p>Every number in its place.</p></div><button className="primary-button" onClick={() => openSheet('new')}>Play again</button></div> : <>
        <div className="tools" aria-label="Puzzle tools">
          ...
          <button className={`tool-button ${pencil ? 'tool-active' : ''}`} aria-pressed={pencil} disabled={paused || busy} onClick={() => setNoteMode(mode => mode === 'note' ? 'value' : 'note')} title="Notes (N)"><span className="notes-icon"><Icon name="pencil"/><span className="notes-badge">{pencil ? 'On' : 'Off'}</span></span><span>Notes</span></button>
          <button className={`tool-button ${excluding ? 'tool-active' : ''}`} aria-pressed={excluding} disabled={paused || busy} onClick={() => setNoteMode(mode => mode === 'exclude' ? 'value' : 'exclude')} title="Exclude (X)"><span className="exclude-icon" aria-hidden="true">4</span><span>Exclude</span></button>
          <button className="tool-button" disabled={!game || paused || busy} onClick={() => setGame(current => current ? fillNotes(current) : current)} title="Fill notes"><Icon name="fill"/><span>Fill notes</span></button>
        </div>
        <div className={`number-pad ${noteMode !== 'value' ? 'pencil-mode' : ''}`} aria-label="Number pad">
          {DIGITS.map(n => {
        ...
        <p className="input-hint">{excluding ? 'Exclude on. Tap a number to rule it out or restore it.' : pencil ? 'Notes on. Tap a number to add or remove a note.' : game?.givens[selected] ? 'Choose an empty cell to add a number.' : 'Select a cell, then a number.'}</p>
      </>}
