...
type NoteProps = { manual: number[]; automatic: number[] };

export function CellNotes({ manual, automatic, filled }: NoteProps & { filled: boolean }) {
  const grid = useRef<HTMLSpanElement>(null);
  const previous = useRef<NoteProps>({ manual, automatic });

  useLayoutEffect(() => {
    const before = previous.current;
    previous.current = { manual, automatic };
    if (filled) {
    ...
    for (const digit of DIGITS) {
      const wasVisible = before.manual.includes(digit) || before.automatic.includes(digit);
      const isVisible = manual.includes(digit) || automatic.includes(digit);
      ...
      const automaticChange = isVisible ? automatic.includes(digit) : before.automatic.includes(digit);
  ...
  return <span className="notes" ref={grid} aria-hidden="true" hidden={filled}>
    {DIGITS.map(digit => <span key={digit} className="note-digit" data-visible={manual.includes(digit) || automatic.includes(digit)}>{digit}</span>)}
  </span>;
}
